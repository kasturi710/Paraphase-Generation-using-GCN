...
DEP_LABELS = {label:ix for ix, label in enumerate(DEP_LABELS)}
    ...
    for d, de in enumerate(deps):
        ...
        for a, arc in enumerate(de):
            if arc[0] != 'ROOT' and arc[0].upper() in DEP_LABELS:         
                ...
                adj_lab_in[idx_in] = np.array([DEP_LABELS.get(arc[0],0)])  # incoming arcs

                mask_in[idx_in] = 1.

                if tmp_out[arc_2] < max_degree:
                    adj_arc_out[idx_out] = np.array([d, arc_1])  # outgoing arcs
                    adj_lab_out[idx_out] = np.array([DEP_LABELS.get(arc[0],0)])  # outgoing arcs
                    mask_out[idx_out] = 1.
